  ...
  constructor() {
    super({
      ...
      void main() {
        ...
        float alpha = (1.2 - clamp(vDistance, 0.0, 1.0)) * clamp(smoothstep(-0.5, 0.25, vPosY), 0.0, 1.0) * uOpacity * revealMask * uRevealProgress * sparkleBrightness * 1.3;

        vec3 jewelryColor = vec3(1.0, 0.75, 0.85);
        gl_FragColor = vec4(jewelryColor, mix(alpha, sparkleBrightness - 0.9, uTransition));
      }`,
      uniforms: {
