...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    const fetchShippingSettings = async () => {
      ...
      const { data, error } = await supabase.from("store_settings").select("standard_shipping_fee, free_shipping_threshold").single()
      ...
      if (error) {
        ...
        setShipping(data.standard_shipping_fee)
  ...
  // ✅ Calculate total with free shipping logic
  const finalShipping = subtotal >= (freeShippingThreshold || 0) ? 0 : (shipping || 0)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        .insert([
          {
            ...
            subtotal,
            delivery_charges: finalShipping,
      ...
      if (isAuthenticated && user?.email) {
        ...
        await supabase.from("profiles").update({ orders: updatedOrders }).eq("email", user.email)
      }
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      ...
      {loadingShipping ? (
        <p className="text-center text-muted-foreground">Loading shipping details...</p>
      ) : (
              ...
              <Card className="sticky top-24">
                ...
                <CardContent className="space-y-6">
                  ...
                  <div className="space-y-3 text-sm">
                    ...
                    <div className="flex justify-between">
                      <span>Shipping</span>
                      <span className={finalShipping === 0 ? "text-green-600 font-medium" : ""}>
                        {finalShipping === 0 ? "FREE" : `Rs. ${finalShipping.toLocaleString()}`}
                      </span>
                    </div>
                    {finalShipping > 0 && freeShippingThreshold && (
